...
class Instance():
    ...
    def load_initial_solutions(self) -> None:
        """
        Loads initial solutions.
        It must be executed only one timer per instance.
        """

        # Get parameters
        params = Params()

        # File path of initial solutions
        file_path = params["FILE_PATH_INITIAL_SOLUTIONS"]

        with open(file_path + "/" + self.file_name + ".txt", "r") as f:
            # Read solutions
            data = f.readlines()
    ...
    def load_problem(self, problem_type: str, file_name: str,
                    create_solutions: bool=True) -> None:
        """
        Loads an instance file.

        @param problem_type: QAP or TSP.
        @param file_name: Instance file path.
        @param create_solutions: True if new initial solutions are required.
        """
